...
CREATE SEQUENCE DEPTS_SEQ
    ...
    START WITH 90
    MAXVALUE 100
    NOCYCLE -- 시퀀스가 MAX에 도달하면 다시 시작부터 하겟는가?
...
--------------------------------------------------------------------------------
--인덱스
--INDEX는 PK, UK 제약에서 자동으로 생성됩니다. 조회를 빠르게하는 HINT역할을 합니다.
--INDEX종류로는 고유인덱스, 비고유인덱스 가 있습니다.
--고유인덱스(UNIQUE) 유니크한 컬럼에만 지정될 수 있습니다.
--비고유인덱스 는 일반컬럼에 지정할 수 있습니다.
...
--인덱스는 여러 컬럼을 묶어서(결합 인덱스) 생성할 수도 있습니다
...
-- 인덱스는 조회를 빠르게 하지만, 무작위하게 많이 사용하는 것은 성능저하를 부를 수 있습니다.
...
SELECT TO_CHAR(SYSDATE, 'YYYY-MM') || '-' || LPAD(시퀀스, 5, 0)
FROM DUAL;
        
INSERT INTO DEPTS2 
VALUES( TO_CHAR(SYSDATE, 'YYYY-MM') || '-' || LPAD(DEPTS_SEQ2.NEXTVAL, 5, 0)
        , 'TEST' );        
...
ALTER SEQUENCE DEPTS_SEQ2 INCREMENT BY -7 MINVALUE 0;
